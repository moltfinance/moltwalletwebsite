...
**Recommended (fast + safe via Clawhub):**

```bash
npx clawhub@latest install moltwallet
```

**Fallback (git clone) — use only if Clawhub is unavailable:**

> Warning: installing from clone can break if deps change or your environment is missing packages.
